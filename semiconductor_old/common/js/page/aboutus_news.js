...
$(window).load(function(){
	$('*[data-role=ui-filter-list]').each(function(a) {
		(function(filter){
			...
			var initYear = function($elmt){
				...
				for(var i=now.getFullYear(); i>=1996; i--){
	...
	$('button[data-role=ui-btn-news-loadmore]').click(function(){
...
function getFilterData(p_page){
	...
	var callback = function(data){
		var $btnLoadmore = $('button[data-role="ui-btn-news-loadmore"]');
		var $filterCnt = $('a[data-role="ui-filter-toggle"]').find('>span>em');
		var $areaList = $('ul[data-tmpl="news"]');
		
		$areaList.find('li').removeClass('odd even');
		$areaList.find('li:odd').addClass('even');
		$areaList.find('li:even').addClass('odd');
	...
	var param = {
		chkNewsContsList : arrVals.toString(),
	...
	// 20150714 news -> press-release 로 명칭 변경
	_common.makeTemplate('/semiconductor/' + SITE_CD + '/about-us/press-release/selectAboutUsNewsSearchList', 'news', options);
}