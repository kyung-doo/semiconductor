...
var DateUtil = {
	...
	set_params : function(y, m, d) {
		return {
			'year' : y,
			'month' : (StringUtil).add_zero(Number(m + 1), 2),
			'date' : (StringUtil).add_zero(d, 2)
		};
	},
...
var StringUtil = {
	...
	to_date : function(value, sign) {
		var msg = '';
		var sign = (!(ValidationUtil).is_null(sign) || sign == '') ? sign : '-';
		// '' 일 때 처리 추가
	...
	get_random : function(type, range) {
		...
		var arr = (ArrayUtil).get_random((chars.length >= range) ? chars.length : range);
		// 전체가 썩이기 위해서 범위(range)가 문자열보다 작으면 문자열 길이로 랜덤 배열 취득
		var msg = '';
		...
		for (var a = 0, atotal = input.length; a < atotal; a++) {
			...
			if (chars[a] >= 0xAC00 && chars[a] <= 0xD7A3) {
				...
				if (isfull) {
					...
					if (i3 != 0x0000)
						v.push(String.fromCharCode(jong_seong[parseInt(i3)]));
				};
			} else {
...
var ValidationUtil = {
	...
	is_null : function(value) {
		...
		if ( typeof (value) == 'undefined' || value == null) {
			bool = true;
		} else if ( typeof (value) == 'string' && ($ || window.jQuery).trim(value) == '') {
			bool = true;
		}
		;
		return bool;
	...
	/**
	 * define-key-down-code (function-key)
	 *
	 * @description - FUNCTION 키 여부 확인
	 * @param	{Number} code
	 * @return	{Boolean}
	 */
	...
	is_mobile : function() {
		var bool;
		var ua = navigator.userAgent;
		if (ua.match(/iPhone|iPod|iPad|Android|Windows CE|BlackBerry|Symbian|Windows Phone|webOS|Opera Mini|Opera Mobi|POLARIS|IEMobile|lgtelecom|nokia|SonyEricsson/i) != null || ua.match(/LG|SAMSUNG|Samsung/) != null) {
			bool = true;
		} else {
			bool = false;
		}
	...
	is_period_date : function(type, sdate, edate) {
		 ...
		 */
		var stime = (!this.is_null(sdate)) ? Number(String(sdate).replace(/[^0-9]/g, '')) : -1;
		var etime = (!this.is_null(edate)) ? Number(String(edate).replace(/[^0-9]/g, '')) : -1;
		...
		if (this[(ismonth)?'is_month_date_nodash':'is_date_nodash'](String(stime)) && this[(ismonth)?'is_month_date_nodash':'is_date_nodash'](String(etime))) {
			/**
			 *  기간 유효성 검사
			 */
			if (stime <= etime) {
